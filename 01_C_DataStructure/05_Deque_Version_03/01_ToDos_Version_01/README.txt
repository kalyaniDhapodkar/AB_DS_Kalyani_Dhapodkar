...
Description :
	...
	1) mv_deque.h  : This header files contains all the declaration of interface functions
			 of 'mv_deque'
...
---------2) deque here is implemented using dcll, on server 
			side deque is using dcll to enque, deque, print and destory 
			data in deque.

			so now it behaves as a server for client but as a client
			for mv_dcll_server.c


	3) mv_client.c : This source file contains interface function calls made 
	...
	5) mv_dcll_server.c : This Source file contains all the function definitions declared in mv_dcll_header.h
